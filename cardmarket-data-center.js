...
(() => {
  ...
  async function importCardmarketProductCatalogPayload(payload, source="products_singles_3.json") {
    ...
    await cmWriteRows("products", prepared, (done,total) => {
      const pctValue = total ? done/total*100 : 100;
    ...
    saveState();
    renderAll();
    cmSetProgress(`<strong>Produktkatalog importiert</strong><br>${prepared.length.toLocaleString("de-DE")} Produkte gespeichert${skipped ? ` · ${skipped} Zeilen übersprungen` : ""}.`, 100, "success");
    return {type:"catalog", rows:rows.length, cards:prepared.length, skipped};
  }
  ...
  async function importCardmarketPriceGuidePayload(payload, source="price_guide_3.json") {
    ...
    await cmWriteRows("latestPrices", latestRows, (done,total) => {
      const pctValue = total ? 70 + done/total*30 : 100;
    ...
    cmSetProgress(`<strong>Price Guide importiert</strong><br>${historyRows.length.toLocaleString("de-DE")} Preise für ${snapshotDate} gespeichert · ${updatedWatchlist} Watchlist-Einträge aktualisiert.`, 100, "success");
    return {type:"prices", rows:rows.length, cards:historyRows.length, updated:updatedWatchlist, snapshotDate, skipped};
  }
