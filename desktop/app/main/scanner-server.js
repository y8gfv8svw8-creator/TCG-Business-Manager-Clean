...
function scannerPage(token) {
  ...
  <div class="steps">1. Karte gerade und vollständig fotografieren.<br>2. Auf gute Beleuchtung achten.<br>3. Falls möglich Setnummer oder Kartenname ergänzen.<br>4. Die genaue Druckvariante anschließend am PC bestätigen.</div>
  ...
  photo.onchange=()=>{const file=photo.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{const image=new Image();image.onload=()=>{const max=1500,scale=Math.min(1,max/Math.max(image.width,image.height)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(image.width*scale));canvas.height=Math.max(1,Math.round(image.height*scale));canvas.getContext('2d').drawImage(image,0,0,canvas.width,canvas.height);imageDataUrl=canvas.toDataURL('image/jpeg',.82);preview.src=imageDataUrl;preview.style.display='block';send.disabled=false;status.textContent='Foto bereit.';status.className='status';};image.src=reader.result;};reader.readAsDataURL(file);};
  send.onclick=async()=>{send.disabled=true;status.textContent='Wird übertragen …';status.className='status';try{const response=await fetch('/submit?token='+encodeURIComponent(token),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({imageDataUrl,hint:document.getElementById('hint').value})});const result=await response.json();if(!response.ok)throw new Error(result.error||'Übertragung fehlgeschlagen');status.textContent='Übertragen. Bitte die Druckvariante am PC bestätigen.';status.className='status ok';photo.value='';imageDataUrl='';preview.style.display='none';}catch(error){status.textContent=error.message;status.className='status bad';}finally{send.disabled=!imageDataUrl;}};</script></body></html>`;
}
...
class ScannerServer {
  ...
  async start(mode = 'business', targetId = '') {
    ...
    return { running: true, sessionId, mode: this.session.mode, targetId: this.session.targetId, url, localUrl: `http://127.0.0.1:${port}/?token=${token}`, addresses, port, qrDataUrl, expiresAt: new Date(Date.now() + SESSION_LIFETIME_MS).toISOString() };
  }

  status() {
    if (!this.server || !this.session) return { running: false };
    return { running: true, sessionId: this.session.sessionId, mode: this.session.mode, startedAt: this.session.startedAt };
  }
  ...
  handleRequest(request, response) {
    ...
    request.on('end', () => {
      try {
        ...
        return this.reply(response, 200, { ok: true, submissionId: submission.id });
      } catch {
