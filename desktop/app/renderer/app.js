...
function collectionPrintMappingStatus(recognition={},observation={}){
  if(observation.printConfidence==="confirmed"&&cleanProductId(observation.selectedProductId))return `Manuell bestätigt · CM ${observation.selectedProductId}. Automatische Prüfungen überschreiben diese Auswahl nicht.`;
  const labels={
    unreadable_set_code:"Setcode nicht lesbar · Ungeprüft - bitte selbst prüfen.",
    ambiguous_ocr_set_code:"Mehrere bekannte Setcodes bleiben möglich · manuelle Prüfung oder Detailfoto erforderlich.",
    reference_unavailable:"Lokale Print-Referenz nicht verfügbar · Ungeprüft - bitte selbst prüfen.",
    no_reference_match:"Kein Referenzprint gefunden · Ungeprüft - bitte selbst prüfen.",
    rarity_selection_required:"Mehrere Raritäten vorhanden · Rarität manuell auswählen. Kein Print wurde bestätigt.",
    version_selection_required:"Mehrere verifizierte Cardmarket-Versionen vorhanden · Version manuell auswählen. Kein Print wurde bestätigt.",
    unverified_rarity:"Dieser Raritätswert ist keiner Version sicher zugeordnet · Ungeprüft - bitte selbst prüfen.",
    unique_candidate:"Genau ein verifizierter Kandidat wird vorgeschlagen. Er ist noch nicht manuell bestätigt.",
    exact_candidate_selected:"Verifizierter Kandidat ausgewählt. Erst „Diesen Print bestätigen“ bestätigt die Cardmarket-ID.",
    exact_reference_without_verified_variant:"Exact-Referenz vorhanden, aber keine vollständig verifizierte Variantenauswahl. Bitte selbst prüfen.",
    likely_candidate:"Mapping-Status likely · Ungeprüft - bitte selbst prüfen.",
    unresolved_candidates:"Mapping-Status unresolved · Ungeprüft - bitte selbst prüfen."
  };
  return labels[recognition.resolutionStatus]||"Noch kein Setcode geprüft.";
}
function collectionReferenceCandidateLabel(candidate={}){
  return [candidate.setName,candidate.setCode||candidate.collectorNumber,candidate.rarity,candidate.version,candidate.treatment].filter(Boolean).join(" · ");
}
...
function renderCollectionPrintRecognition(observation){
  ...
  const referenceCandidates=[...(recognition.variantCandidates||[]),...(recognition.referenceCandidates||[])],selectedId=recognition.selectedCandidateId||recognition.suggestedCandidateId||"",selectedCandidate=referenceCandidates.find(candidate=>candidate.id===selectedId)||null;
  version.innerHTML=`<option value="">${recognition.requiresVersionSelection?"Bitte Version auswählen":"Keine verifizierte Version gewählt"}</option>${(recognition.variantCandidates||[]).map(candidate=>`<option value="${escapeHtml(candidate.id)}" ${candidate.id===selectedId?"selected":""}>${escapeHtml(candidate.version||"Version unbekannt")} · ${escapeHtml(candidate.rarity||"")}</option>`).join("")}`;
  version.disabled=protectedPrint||!(recognition.variantCandidates||[]).length;
  treatment.value=selectedCandidate?.treatment||"";productId.value=protectedPrint?observation.selectedProductId:(selectedCandidate?.verified&&cleanProductId(selectedCandidate.cardmarketProductId)?selectedCandidate.cardmarketProductId:"");
  const recognizedName=observation.selectedName||(observation.nameCandidates||[])[0]?.name||"nicht erkannt";
  status.innerHTML=`<strong>Erkannter Name: ${escapeHtml(recognizedName)}</strong><br><span>Mapping-Status: ${escapeHtml(recognition.mappingStatus||"unresolved")}</span><br><small>${escapeHtml(collectionPrintMappingStatus(recognition,observation))}</small>${recognition.detailPhotoStatus==="detail-photo-needed"?'<br><small class="warning-text">Status: detail-photo-needed · Grund: setcode_unreadable</small>':""}`;
  candidatesElement.innerHTML=referenceCandidates.length?referenceCandidates.map(candidate=>{
    const confirmed=Boolean(observation.printConfidence==="confirmed"&&cleanProductId(candidate.cardmarketProductId)===cleanProductId(observation.selectedProductId)),selected=candidate.id===recognition.selectedCandidateId||confirmed;
    const safe=Boolean(candidate.verified&&cleanProductId(candidate.cardmarketProductId));
...
function renderCollectionObservationEditor(analysis,observation){
  ...
  renderCollectionPrintRecognition(observation);
...
async function recognizeCollectionObservationName(analysis,photo,observation,{dataUrl="",render=true,persist=true}={}){
  ...
  try{
    const preferredSource=window.TcgCollectionPhotoModel?.choosePreferredRecognitionSource?.(analysis,observation)||{photo,observation};
    const sourcePhoto=preferredSource.photo||photo,sourceObservation=preferredSource.observation||observation;
    const sourceDataUrl=sourcePhoto.id===photo.id&&dataUrl?dataUrl:await loadCollectionPhotoData(sourcePhoto);if(!sourceDataUrl)throw new Error("Das Sammlungsfoto konnte nicht geladen werden.");
    const prepared=await window.TcgScannerImageProcessing.prepareCollectionObservationRecognitionPayload(sourceDataUrl,sourceObservation);
    if(prepared.skipRecognition){
      mergeAutomaticCollectionNameCandidates(observation,[]);if(observation.nameConfidence!=="confirmed")observation.nameConfidence="unknown";window.TcgCollectionPhotoModel?.mergeAutomaticPrintRecognition?.(observation,{mappingStatus:"unresolved",resolutionStatus:"unreadable_set_code",setCodeSignals:[],referenceCandidates:[],variantCandidates:[],rarityOptions:[],detailPhotoStatus:"detail-photo-needed",detailPhotoReason:"setcode_unreadable"});if(observation.printConfidence!=="confirmed"){observation.detailPhotoRequired=true;observation.detailPhotoStatus="detail-photo-needed";observation.detailPhotoReason="setcode_unreadable";}observation.nameRecognition={lastRunAt:new Date().toISOString(),engine:"skipped-geometric-check",durationMs:0,confidenceScore:0,margin:0,readings:[],setCodes:[],setCodeSignals:[],passcodes:[],artworkFingerprints:[],conflicts:[],sourcePhotoId:sourcePhoto.id||"",message:prepared.message||"Kartenfläche prüfen: Die Namens-OCR wurde nicht ausgeführt."};observation.updatedAt=new Date().toISOString();if(persist)saveState();return {candidates:[],nameConfidence:"unknown",confidenceScore:0,skipped:true,skipReason:prepared.skipReason};
    }
    ...
    const ocr=result.ocr||{},readings=(ocr.titleReadings||[]).map(row=>String(row.text||"").trim()).filter(Boolean),setCodes=ocr.setCodes||[],setCodeSignals=ocr.setCodeSignals||[],passcodes=ocr.passcodes||[],artworkFingerprints=ocr.artworkFingerprints||[];
    window.TcgCollectionPhotoModel?.mergeAutomaticPrintRecognition?.(observation,{...(result.printRecognition||{}),setCodeSignals,updatedAt:new Date().toISOString()});
    ...
    if(setCodes.length){const bestSetCode=setCodeSignals[0];observation.recognitionSignals.push(`OCR-Setcode: ${setCodes.join(" | ")}${bestSetCode?` · Confidence ${Number(bestSetCode.confidence||0).toFixed(1)} %`:""} · lokal referenzvalidiert`);}else observation.recognitionSignals.push("OCR-Setcode: unlesbar oder nicht eindeutig · Detailfoto erforderlich");
    ...
    observation.nameRecognition={lastRunAt:new Date().toISOString(),engine:String(ocr.engine||"tesseract-local-regions"),durationMs:Number(ocr.durationMs||0),confidenceScore:Number(result.confidenceScore||0),margin:Number(result.margin||0),readings:(result.readings||[]).map(row=>({text:row.text,confidence:row.confidence,variant:row.variant})),setCodes:[...setCodes],setCodeSignals:setCodeSignals.map(row=>({...row})),setCodeValidation:ocr.setCodeValidation?{...ocr.setCodeValidation}:undefined,passcodes:[...passcodes],artworkFingerprints:artworkFingerprints.map(row=>({...row})),conflicts:[...(result.conflicts||[])],sourcePhotoId:sourcePhoto.id||"",sourceObservationId:sourceObservation.id||"",usedPreferredDetailSource:sourcePhoto.id!==photo.id,message};
    if(!setCodes.length&&observation.printConfidence!=="confirmed"){observation.detailPhotoRequired=true;observation.detailPhotoStatus="detail-photo-needed";observation.detailPhotoReason="setcode_unreadable";}else if(setCodes.length&&observation.detailPhotoReason==="setcode_unreadable"){observation.detailPhotoRequired=false;observation.detailPhotoStatus="";observation.detailPhotoReason="";}
...
async function refreshCollectionPrintRecognition(observation,rarity=""){
  if(!observation?.printRecognition?.setCode||!window.desktopApp?.resolveCollectionPrintRecognition)return null;
  const previous=observation.printRecognition,result=await window.desktopApp.resolveCollectionPrintRecognition({setCode:previous.setCode,setCodeConfidence:previous.setCodeConfidence,rarity});
  if(!result?.ok)throw new Error(result?.error||"Die lokale Print-Referenz konnte nicht abgefragt werden.");
  window.TcgCollectionPhotoModel?.mergeAutomaticPrintRecognition?.(observation,{...result.recognition,setCodeSignals:previous.setCodeSignals||[],selectedRarity:rarity,updatedAt:new Date().toISOString()});
...
function stockRowMetadata(r) {
  ...
  const sourceLanguage=mapLanguage(getAny(r,["idLanguage","Language","Sprache"]));
  const sourceCondition=mapCondition(getAny(r,["Condition","condition","Condition-Code","Zustand"]));
  return {
    ...resolved,
    language:sourceLanguage||resolved.language,
    condition:sourceCondition||resolved.condition,
    edition:mapStockEdition(getAny(r,["First Edition","FirstEdition","IsFirstEd","FirstEd","firstEdition","Edition","Erstausgabe"]))
  };
}
...
function mapStockEdition(value){
  const normalized=String(value??"").trim().toLowerCase();
  if(!normalized||["-","n/a","unknown","unbekannt"].includes(normalized))return "Unbekannt";
  if(["1","true","yes","ja","1st","1st edition","first edition","erste auflage"].includes(normalized))return "1st Edition";
  if(["0","false","no","nein","unlimited","unlimited edition","unlimitiert"].includes(normalized))return "Unlimited";
  if(["limited","limited edition","limitiert"].includes(normalized))return "Limited Edition";
...
function removeImport(importRecord, ask=true) {
  ...
  if(importRecord.type==="inventory"&&importRecord.snapshotSync) {
    ...
    const purchaseAssignedIds=new Set(importRecord.purchaseAssignedIds||[]);
    const protectedPurchaseItems=state.inventory.filter(item=>purchaseAssignedIds.has(item.id)&&(["Reserviert","Verkauft"].includes(item.status)||item.saleId));
    if(importRecord.purchaseId&&protectedPurchaseItems.length){
      alert(`Dieser Import kann nicht mehr vollständig rückgängig gemacht werden, weil ${protectedPurchaseItems.length} dem Ankauf zugeordnete Exemplare inzwischen reserviert oder verkauft sind. Bestand und Ankauf bleiben unverändert.`);
      return;
    }
    ...
    if(importRecord.purchaseId&&!protectedCreated){
      if(importRecord.purchaseCreated)state.purchases=state.purchases.filter(purchase=>purchase.id!==importRecord.purchaseId);
      else if(importRecord.purchaseBefore){
        const purchaseIndex=state.purchases.findIndex(purchase=>purchase.id===importRecord.purchaseId);
        if(purchaseIndex>=0)state.purchases[purchaseIndex]=structuredClone(importRecord.purchaseBefore);
      }
    }
    ...
    if(protectedCreated)undoNotice=`${protectedCreated} inzwischen reservierte oder verkaufte Exemplare wurden zum Schutz der Verkaufsdaten nicht entfernt. Die zugehörige Ankaufshistorie blieb deshalb ebenfalls erhalten.`;
...
function prepareStockSnapshotRows(rows=[]){
  ...
  rows.forEach((r,idx)=>{
    ...
    if(skipQty){pendingOpen.set(variantKey,pendingQty-skipQty);skippedOpenOrders+=skipQty;}
    const articleId=cleanProductId(getAny(r,["ArticleID","idArticle"]));
    if((!p.edition||p.edition==="Unbekannt")&&articleId){
      const existing=state.inventory.find(item=>cleanProductId(item.articleId||item.idArticle)===articleId);
      if(existing?.edition)p.edition=existing.edition;
    }
  ...
  const acquisitionRows=[...snapshotRows.values()].map(row=>({
    ...row.p,
    articleId:row.articleId,
    quantity:row.quantity,
    listingPrice:row.offerPrice,
    sourceRow:row.sourceRow
  }));
  return {snapshotRows,cards,unknown,skippedOpenOrders,acquisitionRows};
}

function stockPurchaseExistingCost(purchaseId=""){
...
async function importStock(file,options={}) {
  ...
  const prepared=prepareStockSnapshotRows(rows);
  const {snapshotRows,cards,unknown,skippedOpenOrders}=prepared;
  let stockPurchasePreview=null,stockPurchase=null,stockPurchaseBefore=null;
  const stockPurchaseOptions=options.stockPurchase||{};
  if(stockPurchaseOptions.enabled){
    stockPurchase=stockPurchaseOptions.purchaseId?state.purchases.find(row=>String(row.id)===String(stockPurchaseOptions.purchaseId)):null;
    if(stockPurchaseOptions.purchaseId&&!stockPurchase)throw new Error("Der ausgewählte Ankauf ist nicht mehr vorhanden.");
    if(stockPurchase&&stockPurchaseHasOpenLines(stockPurchase.id))throw new Error("Der ausgewählte Ankauf enthält noch offene Wareneingangspositionen und kann deshalb nicht automatisch ergänzt werden.");
    ...
    stockPurchaseBefore=stockPurchase?structuredClone(stockPurchase):null;
  }

  const importStateBefore=structuredClone(state);
  try {
  ...
  snapshotRows.forEach(row=>{
    ...
    const metadata={
      ...
      set:row.p.set,setName:row.p.setName,variant:row.p.variant||"",rarity:row.p.rarity,language:row.p.language,condition:row.p.condition,edition:row.p.edition||"Unbekannt",
  ...
  let purchaseAllocation=null;
  if(stockPurchasePreview){
    const purchaseId=stockPurchase?.id||uid();
  ...
  state.imports.push({
    ...
    added:createdIds.length,removed:removedItems.length,purchaseId:purchaseAllocation?.purchase?.id||"",
    purchaseCreated:Boolean(purchaseAllocation&&!stockPurchaseBefore),purchaseBefore:stockPurchaseBefore,
    purchaseAssignedIds:purchaseAllocation?.assignedIds||[]
  });
...
async function universalImportFile(file,options={}) {
  ...
  if (type === "inventory") return {...await importStock(file,{stockPurchase:options.stockPurchase||{}}),type:"inventory"};
  ...
  try{
    if(lower.endsWith(".json")){
      ...
      if(analysis.type==="inventory"){
        analysis.key=`STOCK-${file.name}-${Number(file.size||0)}-${Number(file.lastModified||0)}-${rows.length}`;
        const resolution=await resolveMissingStockProductIds(rows);
        const prepared=prepareStockSnapshotRows(rows);
        analysis.stockAcquisitionRows=prepared.acquisitionRows;
        analysis.stockProductIdsResolved=resolution.resolved;
        analysis.stockProductIdsUnresolved=resolution.unresolved;
      }
      ...
      if(analysis.type==="inventory"){
        analysis.warnings.push("Vollständiger Abgleich: Neue Positionen werden ergänzt, nicht mehr angebotene Cardmarket-Positionen aus dem aktuellen Bestand entfernt. Verkaufs- und Stornohistorie sowie manuelle/private Bestände bleiben erhalten.");
        if(analysis.stockProductIdsUnresolved)analysis.warnings.push(`${analysis.stockProductIdsUnresolved} Position(en) haben keine eindeutige Cardmarket-Produkt-ID und werden keinem Ankauf automatisch zugeordnet.`);
      }
    }
...
function stockPurchaseSelectorOptions(){
  return [
    '<option value="new">Neuen Ankauf anlegen</option>',
    ...state.purchases.filter(purchase=>purchase.status!=="Storniert").map(purchase=>{
      const label=[purchase.title||purchase.seller||"Ankauf",purchase.orderNo,purchase.date?fmtDate(purchase.date):"",money(purchase.cardValue||0)].filter(Boolean).join(" · ");
      return `<option value="${escapeHtml(purchase.id)}">${escapeHtml(label)}</option>`;
    })
  ].join("");
}
...
function stockPurchaseControlsHtml(item){
  ...
  return `<section class="import-stock-purchase" data-stock-purchase-section="${item.id}">
    ...
    <div class="import-stock-purchase-fields">
      <label>Ankauf<select data-stock-purchase-select="${item.id}" disabled>${stockPurchaseSelectorOptions()}</select></label>
      <label>Bezeichnung<input data-stock-purchase-title="${item.id}" value="${escapeHtml(suggested)}" disabled></label>
...
function stockPurchaseUnitCostText(row){
  const groups=new Map();
  (row.unitCosts||[]).forEach(value=>{const key=Number(value).toFixed(2);groups.set(key,(groups.get(key)||0)+1);});
  if(!groups.size)return "–";
  return [...groups.entries()].map(([value,count])=>`${count}× ${money(value)}`).join(" · ");
}

function refreshStockPurchasePreview(itemId){
  const item=pendingImportBatch.find(row=>row.id===itemId);
  if(!item||item.type!=="inventory")return;
  const enabled=Boolean(document.querySelector(`[data-stock-purchase-enabled="${item.id}"]`)?.checked);
  const select=document.querySelector(`[data-stock-purchase-select="${item.id}"]`);
...
function refreshImportConfirmState(){
  ...
  const selected=pendingImportBatch.filter(item=>!item.blocked&&item.type!=="unknown"&&document.querySelector(`[data-import-approved="${item.id}"]`)?.checked);
  confirmButton.disabled=!selected.length||selected.some(item=>{
    const enabled=document.querySelector(`[data-stock-purchase-enabled="${item.id}"]`)?.checked;
    return item.type==="inventory"&&enabled&&(!item.stockPurchaseInputComplete||!item.stockPurchasePreview?.canApply);
  });
}
...
async function confirmImportPreview() {
  ...
  for(const item of selected){
    try{
      ...
      const stockEnabled=forcedType==="inventory"&&document.querySelector(`[data-stock-purchase-enabled="${item.id}"]`)?.checked;
      const selectedPurchase=document.querySelector(`[data-stock-purchase-select="${item.id}"]`)?.value||"new";
      const stockPurchase=stockEnabled?{
        enabled:true,
        purchaseId:selectedPurchase==="new"?"":selectedPurchase,
...
document.getElementById("importPreviewContent").addEventListener("input",event=>{
  const field=event.target.closest("[data-stock-purchase-title],[data-stock-purchase-total]");
  if(!field)return;
  const itemId=field.dataset.stockPurchaseTitle||field.dataset.stockPurchaseTotal;
  refreshStockPurchasePreview(itemId);
});
