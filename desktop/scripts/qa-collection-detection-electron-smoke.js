...
async function main() {
  ...
  try {
    ...
    const result = await evaluate(page.webSocketDebuggerUrl, `(async()=>{
      ...
      const evaluateImage=async target=>{const detection=await TcgScannerImageProcessing.detectCollectionCardsFromDataUrl(target.node.toDataURL('image/png'));return {...TcgScannerImageProcessing.evaluateCollectionDetections(target.expected,detection.detections,.4),detections:detection.detections,quality:detection.photoQuality};};
      const blank=canvas(640,420),single=canvas(720,520),four=canvas(760,560),grid=canvas(780,780,'#303338'),slanted=canvas(700,560);
      ...
      const metrics={blank:await evaluateImage(blank),single:await evaluateImage(single),four:await evaluateImage(four),grid:await evaluateImage(grid),slanted:await evaluateImage(slanted)};
