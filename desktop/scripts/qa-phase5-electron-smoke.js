...
async function main() {
  ...
  try {
    ...
    if(!result||!result.title?.includes('6.9.0')||result.ready!=='complete'||!viewsOk||!result.collectionView||!result.entryFields||!result.germanSearch||!result.englishSearch||!result.priceGuide||!result.maxEk||!result.snapshot||!result.noAutomaticInventory||!result.phase3Bridge||!result.phase4Bridge||result.databaseStatus?.schemaVersion!==11||result.rendererErrors?.length){
