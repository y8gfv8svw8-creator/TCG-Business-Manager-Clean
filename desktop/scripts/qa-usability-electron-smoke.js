...
async function main() {
  ...
  try {
    ...
    const result = await evaluate(page.webSocketDebuggerUrl, `(async()=>{
      ...
      state.inventory=Object.keys(importTotals).map((set,index)=>({id:'qa-price-'+set,collectorNumber:set+'-EN'+String(index+1).padStart(3,'0'),name:'Importkarte '+set,germanName:'Importkarte '+set,status:'Im Bestand',cost:1,costStatus:'known',targetSell:2,originalTargetSell:2,cardPrice:.5,quantity:1,listingHistory:[]}));
