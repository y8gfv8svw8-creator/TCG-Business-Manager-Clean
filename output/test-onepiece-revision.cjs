...
assert.equal(count(),579);el('finish').value='foil';assert.equal(count(),0);
el('finish').value='nonfoil';assert.equal(count(),1);vm.runInContext('render()',ctx);
assert(el('grid').innerHTML.includes('Non-Foil · ohne Holo'));assert(el('grid').innerHTML.includes('★ Favorit'));
...
el('finish').value='unknown';assert.equal(count(),578);vm.runInContext('render()',ctx);assert(el('grid').innerHTML.includes('Ungeprüft – bitte selbst prüfen'));
